...
func save_slot(slot: int = 1) -> bool:
	var state := {
		...
		"inventory": GameState.inventory, "quest": {"id": GameState.active_quest, "stage": GameState.quest_stage},
...
func load_slot(slot: int = 1) -> bool:
	...
	GameState.inventory = data.get("inventory", {}).duplicate(); var quest: Dictionary = data.get("quest", {})
